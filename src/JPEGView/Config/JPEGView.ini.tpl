...
; Number of images to decode in advance (read-ahead) for instant forward/back
; navigation. Default 2; raise to keep more images pre-decoded in memory.
; Range 1 to 8. Higher values use more memory.
ReadAheadBuffers=2
...
; Background mode for transparent images: "Black", "White" or "Checkerboard".
; Can be switched live with the B key or the context menu (Background submenu).
...
; Offload the hot image-processing passes (high-quality resampling, unsharp mask,
; local density correction and LUT/color correction) to a D3D11 compute shader
