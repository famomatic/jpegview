...
public:
	...
	// Legacy alias for cooperative termination. Never force-kills a thread while
	// it may own a lock or allocator state.
	void Abort();
...
protected:
	...
	bool ProcessAndWait(CRequestBase* pRequest);
	...
	bool ProcessAsync(CRequestBase* pRequest);
	...
	std::atomic_bool m_bTerminate; // flags termination for thread

private:
	static unsigned __stdcall ThreadFunc(void* arg);
	static void DeleteAllRequestsMarkedForDeletion(CWorkThread* thisPtr);
	static void SignalRequest(CRequestBase* request);
